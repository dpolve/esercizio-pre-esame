...
?>
    ...
    <body>
        <div class="container-fluid">
            ...
            <h1>
                Statistiche per <a href="menu.php?freq=T&argeve=A"> Argomenti </a>
                o <a href="menu.php?freq=T&argeve=E"> Eventi </a>
            </h1>
            <br>
            <a href="menu.php?freq=T&argeve=A"> Totali</a>
            ,<a href="menu.php?freq=I"> Ieri</a>
            ,<a href="menu.php?freq=O"> Oggi </a>
            <?php
            $today = date("Y-m-d");
            ...
            $freq = $_REQUEST["freq"];
            $argeve = $_REQUEST["argeve"];
            if ($_REQUEST["argeve"] == "A") {
                echo "carica argomenti";
                exit;
            } else {
                echo "carica eventi";
                exit;
            ...
            <?php
                ...
                // 2.1 lettura records di tabella statistiche Ieri per argomento
                $stat_ier =
        ...
        GROUP BY argomenti.arg_argomento ";
            ...
            <?php
                ...
                // 2.1 lettura records di tabella statistiche OGGI per argomento
                $stat_ogg =
                    " SELECT argomenti.arg_id , argomenti.arg_argomento , COUNT(*) as sta_tot
        FROM statistiche 
        INNER JOIN eventi on eventi.eve_id = statistiche.sta_eve_id
        INNER JOIN argomenti ON eventi.eve_arg_id = argomenti.arg_id
        WHERE statistiche.sta_data = '$today'
        GROUP BY argomenti.arg_argomento ";
                $st_stat_ogg = $conn->prepare($stat_ogg);
                $st_stat_ogg->execute();
                $sta_ogg = $st_stat_ogg->fetchAll(PDO::FETCH_ASSOC);

            ?>
                <!-- <pre><?php var_dump($sta_ogg) ?></pre> -->
                <table class="table table-striped">
                    <thead>
                        <tr>
                            <th scope="col">Statistiche di Argomenti</th>
